...
$computerInfoFinal = $dpathfinal + "\Computername.txt"
write-output $settingFormat | Out-File $computerInfoFinal

$regeditPath = "HKLM\TEMP\CONTROLSET001\Services\Eventlog /f Computer"
$computerRegName = REG QUERY $regeditPath
[System.Reflection.Assembly]::LoadWithPartialName("System.Windows.Forms")
[System.Windows.Forms.MessageBox]::Show("Este es el nombre del computador" + $computerRegName)
ForEach ($regValues in $computerRegName){
   $objectWithName = $regvalues | Select-String -Pattern "ComputerName" -quiet 
   if ($objectWithName){
        $stringValue = $regvalues
...
$computerTempName = $dpathfinal + "\ComputerTemporalName.txt"
write-output $arrayComputerName | Out-File $computerInfoFinal
#>